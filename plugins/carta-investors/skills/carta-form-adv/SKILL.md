---
name: carta-form-adv
description: Fetch comprehensive Form ADV Part 1A data for SEC investment adviser registration filings. Covers Schedule D §7.B.(1) per-fund detail, Item 5.D client type AUM rollup, Item 5.F regulatory AUM (discretionary/non-discretionary and account counts), investor demographics (US vs non-US beneficial owners, entity type breakdown), annual capital activity, portfolio asset class composition, and fund formation/structure metadata. Use when asked about Form ADV, regulatory AUM, Schedule D, Form PF Section 1, annual SEC filing data, beneficial owner counts, or private fund disclosures.
publish: true
allowed-tools:
  - mcp__carta__fetch
    ...
    COUNT(DISTINCT pd.partner_id)
        FILTER (WHERE pd.is_limited_partner AND pd.is_active
            AND UPPER(TRIM(pd.partner_country)) IN (
                ...
                'U.S.', 'U.S.A.', 'UNITED STATES OF AMERICA (USA)'))                   AS us_lp_investors,
    COUNT(DISTINCT pd.partner_id)
        FILTER (WHERE pd.is_limited_partner AND pd.is_active
...
After presenting the markdown output, automatically generate a pre-filled Form ADV PDF.
Tell the user: *"Generating your Form ADV preparation PDF..."*

### Step 1 — Build the data file

Extract values from Query 1 and Query 2. Use the `Write` tool to create `/tmp/form_adv_data.json`:

- **`firm_name`** and **`reporting_date`**: from the firm_rollup row
- **`firm_rollup`**: the row where `form_adv_section = '5.D./5.F. Firm Rollup'` — include all numeric columns
- **`funds`**: all rows where `form_adv_section = '7.B.(1) Fund Detail'` — one object per fund with all columns, plus `lp_nav`, `gp_nav`, `annual_lp_subscriptions`, `annual_gp_subscriptions`, `annual_lp_distributions`, `annual_gp_distributions`, `lp_contributions_since_inception`, `lp_distributions_since_inception`
- **`investor_demographics`**: all rows from Query 2, keyed by `fund_uuid`

Use actual values from the query results — no placeholders.

### Step 2 — Write and run the generator script

Use the `Write` tool to write the full contents of `form_adv_pdf_generator.py` (committed alongside this skill) to `/tmp/form_adv_pdf_generator.py`, then run:

```bash
python3 /tmp/form_adv_pdf_generator.py /tmp/form_adv_data.json
```

The script auto-installs `reportlab` if not present. Output path: `~/Downloads/FormADV_<FirmName>_<Year>.pdf`

### Step 3 — Confirm to the user

Tell the user the exact output path printed by the script, then say:

*"Your Form ADV Part 1A preparation package is ready — check your Downloads folder. Pre-filled fields are in black; orange fields must be completed manually in IARD."*
