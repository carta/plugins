...
- **In-app Update-data button (a form).** The console's topbar/sidebar button opens a panel of dataset checkboxes — **Operating KPIs & Forecasts** as one row (the two time-series datasets run as one pull; see `scripts/datasets.py`), then Holdings & returns and Ownership & cap tables — each with its last-fetched time, all checked by default, and **one submit button** whose label names the run (**Refresh all** / **Refresh KPIs & Forecasts** / **Refresh N datasets**; disabled **Select a dataset** when nothing is checked). Submitting POSTs `/api/refresh` (`{}` for all, `{"datasets":[keys]}` for a subset); `serve.py` runs `refresh.py`, which spawns a **sandboxed headless `claude` subprocess** (allowed only `mcp__<prefix>__{welcome,set_context,call_tool}`, no Bash/Write, server-authored SQL) to fetch into the raw dir, streaming progress the app polls at `/api/refresh/status`. The user then clicks **Load new data** (`/api/refresh/apply`) to rebuild `kpi.json` and reload. `/api/capabilities` reports whether the button can run (a `claude` binary on PATH + a firm-id'd cache); when it can't, the panel shows freshness read-only and points at the chat fallback. Freshness is per-stem ndjson mtime, stamped into `kpi.json.source.datasets` + `source.builtAt` by the build. On an escalation (`needs_human`, e.g. a stem past 100,000 rows or an id-less cache) the panel falls back to the chat flow below. A full fetch lands in a `<stem>.ndjson.fetching` staging file and replaces the stem only once every page is in, so a fetch that stops short leaves the previous stem in place; a stem left with a `.truncated` or `.integrity_error` marker refuses to build until it is re-fetched. The refresh asks the DWH for `response_mode: "blob"`, so a 10,000-row page of `financials` (3–4 MB on a large firm) lands on disk and the model sees only an ack + path; every Carta MCP call it issues, including `welcome` and `set_context`, carries `_instrumentation_v2` (the server rejects calls without it, and the plugin hook doesn't run in the subprocess). The warehouse tables refresh every few minutes, and a refresh that lands mid-fetch shifts every later OFFSET page (on QED: 554 rows duplicated, 554 missed); the saver's integrity check catches that and the stem is fetched once more. The whole session is mirrored to `<raw_dir>/refresh.log` — read that first when a refresh fails.
- **The KPIs & Forecasts row can be narrowed by company and date.** Its **Narrow ›** button (rendered only when the firm has a company list and the in-app refresh is available; **Narrowed ›** once set, disabled when the row is unchecked) opens a second page inside the popover: an all-selected-by-default **company multi-select** ("All 298 selected · Clear" ↔ "2 of 298 selected · Select all"; bulk actions act on the searched rows while a search is active) and an optional **start date** ("All history · Set a date" ↔ "Jan 1, 2025 · Clear"). **Confirm** saves the draft back to the form (a summary line restates it: "Will refresh Operating KPIs & Forecasts for 2 companies since Jan 1, 2025."); **‹ Datasets** discards; the row's sub-line then carries the scope ("Updated 2h ago · 2 companies · since Jan 2025"). All-companies-selected sends **no** company filter; a subset is sent as `{"companies":{"ids":[…]}}` — `kpi.json` `companies[].id` values (name-keyed companies are not offered — they have no identity columns to filter by). Both filters narrow **Operating KPIs & Forecasts only**. `refresh.py` resolves the ids to warehouse identity values via the cached identity stems — the same mapping the builder's `company_key()` uses — and `emit_stem_sql.py` injects `AND period_end >= '<since>'` and `AND (general_ledger_issuer_id IN (…) OR corporation_id IN (…) OR llc_entity_id IN (…))` into `financials`/`forecasts` and nowhere else; no display name ever reaches SQL. Ids that don't resolve are skipped (all-unresolvable fails the run). A company-filtered fetch of a stem with a cached base takes a company-scoped merge (`merge_stem_delta.py --replace-companies`, `--since`-aware: drop only the selected companies' in-window rows, append the fresh ones) so every other company's data — and the selected companies' pre-`since` history — survives; a stem with **no** cached base falls back to an unfiltered full fetch so a narrow delta never becomes the whole cache. After any filtered run, `run_fetch` scans the merged cache and returns structured `companyWarnings` (`{id, noRows:[dataset keys]}` per selected company with nothing in the requested window — on a date-only run the population is every identity-keyed company — plus `{id, skipped:true}` per unresolvable id) with `selectedCount` and `filterSince`; the panel renders them grouped per dataset with per-company reason lines, 3 inline plus a **View all N ›** page with reason-filter pills. The filter lives exactly as long as its pull: the resolved filter file is deleted when the run ends and the whole form resets to defaults once the fetch lands. The filters are a **speed lever to pull less**; widening the window still needs a full chat "Refresh KPI data".
