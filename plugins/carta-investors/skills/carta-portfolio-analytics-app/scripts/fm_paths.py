#!/usr/bin/env python3
"""Portfolio Analytics path resolver + filesystem helpers for SKILL.md steps.
...
def cache_root():
    # PORTFOLIO_ANALYTICS_DATA is the current override; KPI_DASHBOARD_DATA is the
    # pre-rename name, still honoured so an existing shell/env keeps working.
    env = os.environ.get("PORTFOLIO_ANALYTICS_DATA") or os.environ.get("KPI_DASHBOARD_DATA")
    if env:
        return pathlib.Path(env)
    return pathlib.Path.home() / ".cache" / "carta-portfolio-analytics-app"


# The dataset file whose presence/mtime marks a built dashboard.
DATASET_FILE = "kpi.json"
...
def snapshot_age_days(slug):
    snap = dashboard_dir(slug) / DATASET_FILE
    if not snap.exists():
        return None
...
def main():
    ap = argparse.ArgumentParser(description="Portfolio Analytics path resolver")
