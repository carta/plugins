// Snowplow UI-event tracking for the portfolio-analytics console via @carta/mcp-ui-tracker
...
// so no one can retarget the firm by editing a URL and no bookmark can drop it. main.jsx awaits
// it before rendering, so no event fires ahead of it.
//
...
// Integers only — the caller passes a JSON number from the server; anything else is an upstream bug.
...
/** Authoritative id from the loaded KPI dataset — follows a refresh that rewrites the firm mid-session. */
...
export async function initPortfolioAnalyticsTracker() {
  ...
  window.mcpUiTracker.initTracker({
    interface: { interfaceType: "micro_app", interfaceId: "carta-portfolio-analytics" },
