      ...
      {!hidePresets && presets.length > 0 && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, alignItems: "center" }}>
          {presets.map((p) => {
            ...
            return (
              <button key={p.label} onClick={() => { if (!disabled) { trackClick("FundModeling.Reprice.Preset"); onChange(p.v); } }} disabled={disabled}
          ...
          {onReset && (
            <button onClick={() => { if (!disabled) { trackClick("FundModeling.Reprice.Reset"); onReset(); } }} disabled={disabled} title="Reset this company to its Carta mark"
