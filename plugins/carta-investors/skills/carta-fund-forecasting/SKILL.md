...
<carta-plugin>carta-investors:6.37.10</carta-plugin>
...
| `fund_forecasting:list:investments` | `fund_id` | `investment_ids`, `period`, `includeIntegrationStatus`, `detail`, `rounds`, `qualitative`, `page`, `page_size` | per-investment KPIs, paginated — response includes `total`, `page`, `page_size`, `has_more`; compact projection by default; `detail=full` for all ~90 raw fields; `rounds=true` adds per-investment `rounds[]` ladder; `qualitative=true` adds qualitative fields; `investment_ids` returns only the requested rows plus `requested_count` and `not_found` |
...
- `list:investments` `investment_ids`: comma-separated `investmentId` values — returns only those rows, in the order requested, plus `requested_count` and `not_found`. **Use this whenever you need more than one specific company**; never loop `get:investment`.
- `list:investments` `page` / `page_size`: paginated on the backend; the response always includes `total`, `page`, `page_size`, `has_more`. Page sizes that fit under the response cap: summary ≈ 15, summary with `qualitative=true` ≈ 8, `detail=full` 1–2.
...
- "**Full record for one or more specific portfolio companies** (all ~90 fields, per-stage follow-on arrays, `stages[]`)?" → `list:investments` with `investment_ids` and `detail=full`, `page_size=1` or `2` (a full row is 16–31K chars). `get:investment` is equivalent for exactly one company.
