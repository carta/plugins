...
> **Why the `norm_investments_base` / `norm_investments` / `norm_spa` /
> `canonical_company` CTEs:** SPA
...
> the SPA spelled it. The `norm_investments_base` CTE also enforces the shared
...
> **Why `norm_investments` emits two candidate names per investment:** an SOI
> record often carries the SPA's name as a parenthetical alias — "NewCo
> (formerly OldCo)", "LegalCo, Inc. (dba BrandCo)", "Topco LLC (fka TargetCo,
> Inc.)". Normalization strips parentheticals, so the primary name normalizes
> to `NEWCO` while the SPA says `OLDCO`, `JAROWINKLER_SIMILARITY` lands
> nowhere near 90, and the company silently falls out of the Live/Exited
> filter (it keeps its `companyStatus` only via a matched SOI name). So
> `norm_investments_base` also pulls the alias out with `REGEXP_SUBSTR(...,
> 'ie', 3)` when the parenthetical opens with an alias marker (`d/b/a`,
> `f/k/a`, `n/k/a`, `aka`, `formerly`, `formerly known as`, dotted forms
> included). **The marker separator is `[ :]+`, not a plain space** — real SOI
> data carries both `(dba BrandCo)` and `(DBA: BrandCo)`, and a space-only
> pattern silently misses every colon form. `norm_investments` then
> `UNION ALL`s the alias in as a **second candidate
> row for the same `INV_NAME`**. `spa_fuzzy` already keeps only the
> highest-scoring row per SPA name (`ROW_NUMBER() ... ORDER BY
> JAROWINKLER_SIMILARITY DESC`), so a second candidate can only *add* a chance
> to match — it never displaces a legitimate match on the primary name, and
> either candidate resolves to the same `INV_NAME`, so `canonical_company`'s
> output is unchanged in shape. A parenthetical that is not an alias marker
> ("(a Delaware corporation)", "(Series B only)") yields NULL and emits no
> extra row. **Keep the coverage counts on `norm_investments_base`, not
> `norm_investments`** — counting the post-`UNION ALL` CTE double-counts every
> aliased company in both the numerator and the denominator.
