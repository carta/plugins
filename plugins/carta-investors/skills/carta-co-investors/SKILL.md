...
Use `fetch("dwh:execute:query", {...})` with the appropriate query below.
...
> If a query fails with a table-not-found error: call `fetch("dwh:list:tables", {})`
