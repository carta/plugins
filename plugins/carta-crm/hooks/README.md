...
The `inject-instrumentation` hook targets the Carta MCP server via an
explicit allowlist regex rather than `mcp__carta.*__.*`, since the server
name varies by how it was registered:

- `[Cc]arta.*` — prefix match; covers any server name starting with "carta" or "Carta" (e.g. `carta-local`)
