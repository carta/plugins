...
| `ownership` | Voting rights, liquidation seniority, and preferred vs common holder analysis. Use when asked about voting power, protective provisions, consent requirements, or seniority. For general visual ownership summaries, use the `cap_table_chart` MCP tool instead. |
...
| `portfolio-query` | Query cap table data across multiple companies, or fetch detailed per-company data (stakeholders, grants, SAFEs, 409A). Use for portfolio-wide analysis, comparing companies, finding patterns, or when detailed tabular data is needed beyond a visual summary. |
