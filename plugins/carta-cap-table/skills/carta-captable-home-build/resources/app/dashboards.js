...
function renderCapTableTile() {
  if (!_capTableChartData) {
    setDashboardBody("captable-dash-body", `<div class="loading-row">Loading…</div>`);
    return;
  }
...
// The stacked bar and the table read the same rows, so they belong on one page: the
// bar carries the composition, the table the exact numbers.
...
function renderRoundHistoryTile() {
  if (_roundHistory === null) {
    setDashboardBody("rounds-dash-body", `<div class="loading-row">Loading…</div>`);
    return;
  }
...
async function fetchRoundHistory(corporationId) {
  ...
  if (errText) {
    ...
    if (_isPermissionError(errText)) {
      setDashboardBody("rounds-dash-body", `<div class="empty-row">You don't have access to the financing history for this company.</div>`);
      return;
    }
    setDashboardBody("rounds-dash-body", `<div class="error-row">Couldn't load the round history.</div>`);
...
// afterShow runs once the page is visible: Chart.js sizes to a laid-out canvas, so a
// chart drawn while the container is display:none comes out zero-height.
function openDashboardPage(pageId, render, trackId, afterShow) {
...
function openCapTablePage() {
  openDashboardPage("captable-page", renderCapTablePage, "CaptableHome.Dashboards.CapTable.Open", drawOwnershipChart);
}
function openRoundsPage() {
  openDashboardPage("rounds-page", renderRoundHistoryPage, "CaptableHome.Dashboards.Rounds.Open");
}
function openOptionPoolPage() {
  openDashboardPage("option-pool-page", renderOptionPoolPage, "CaptableHome.Dashboards.OptionPool.Open");
}
function openStakeholdersPage() {
  openDashboardPage("stakeholders-page", renderStakeholdersPage, "CaptableHome.Dashboards.Stakeholders.Open");
}
function openDraftsPage() {
  openDashboardPage("drafts-page", renderDraftsPage, "CaptableHome.Dashboards.Drafts.Open");
}
